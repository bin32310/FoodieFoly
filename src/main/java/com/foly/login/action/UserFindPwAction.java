...
/*
 *  
 *  UserFindPwAction - 유저 비밀번호 찾기에 필요한 동작 처리(이름, 이메일, 아이디 확인)
 *  
 * */
public class UserFindPwAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		System.out.println(" M : UserFindPwAction_execute() 호출 ");
...
//		request.setCharacterEncoding("UTF-8");
	 ...
	 //  3) DAO 객체 생성 - userFIndPwCheck(us_name,us_email);
		...
		String us_pw = dao.userFindPwCheck(us_name,us_id,us_email);
		if(us_pw.equals("찾는비밀번호없음")) {
			result = -1;
		}
		else {
			result = 1;
		...
		String receiver = us_email;
		String subject = "FoodieFoly 회원님의 비밀번호를 알려드립니다.";
		String content = "회원님의 비밀번호는 "+us_pw+"입니다.";
