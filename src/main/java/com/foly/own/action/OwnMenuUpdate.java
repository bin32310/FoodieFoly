...
public class OwnMenuUpdate implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		
		System.out.println(" M : OwnMenuUpdate_execute() 호출 ");
		...
		System.out.println(pageNum+"값확인");
		// 3) DTO 객체 - 게시판 글수정 정보 저장
		...
		int result = dao.OwnMenuUpdate(dto);
		
		// 4) 수정 메서드 결과에 따른 페이지 이동(JS)
		if(result == -1) {
			
			JSMethod.alertBack(response, "메뉴 정보 없음");
			...
			JSMethod.alertLocation(response, "수정 완료!", "./OwnMenu.ow?pageNum="+pageNum);
